...
def set_html_up():
    ...
    text_file2.write(f"<a href='cont_cont.html'> <h1>Continous/Continous</h1></a></h1>")
    text_file2.write(
        f"<a href='cont_cat.html'> <h1>Categorical/Continous</h1></a></h1>"
    )
    text_file2.write(
        f"<a href='cat_cat.html'> <h1>Categorical/Categorical</h1></a></h1>"
    )
    text_file2.close()
...
def cat_cont_correlation_metrics(categorical, continuous):
    ...
    fig2.write_html(file=f"corr_heatmap_cat_cont.html", include_plotlyjs="cdn")
    fig2.show()
...
def mean_of_resp(df, pred1, pred2, table_df2, file):
    ...
    pred2name = pred2
    # --GET BIN DATA--
    population_mean = df["survived"].mean()
...
def main():
    ...
    for a, col in enumerate(biserial_cat_resp.T):
        ...
        fig2.show()
        mean_of_resp(biserial_cat_resp, f"{col}", f"{feature_name}", cat_cat_bf, "cat-cat")
    ...
    df_bins = pd.cut(cont_resp.index, bins)


# Cont
# - take 2 vars and bin them, see how many are in that bin, each var

# histogram
# - array of values
# - need to find number of bins
# - bin size = divide max - min / bins
# - after bins, find data points and which go to each bin
#     - diff bowls, see which goes in which
# - count is how many data points in these bins
# - two variables
# - then add response
